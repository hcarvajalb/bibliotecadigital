 ...
 */
public class OpenidAction extends AbstractAction
{
    /**
            ...
            String source, Parameters parameters) throws Exception
    {
        ...
        final String AUTHORIZE = DSpaceServicesFactory.getInstance().getConfigurationService().getProperty("org.dspace.app.xmlui.aspect.eperson.openid.authorize");

        final String RETURN_URL = DSpaceServicesFactory.getInstance().getConfigurationService().getProperty("org.dspace.app.xmlui.aspect.eperson.openid.return_url");
        ...
        final String CLIENT_ID = DSpaceServicesFactory.getInstance().getConfigurationService().getProperty("org.dspace.app.xmlui.aspect.eperson.openid.client_id");
        
        final String RESPONSE_TYPE = DSpaceServicesFactory.getInstance().getConfigurationService().getProperty("org.dspace.app.xmlui.aspect.eperson.openid.response_type");
        
        final String SCOPE = DSpaceServicesFactory.getInstance().getConfigurationService().getProperty("org.dspace.app.xmlui.aspect.eperson.openid.scope");

            
                    HttpSession session = request.getSession();
                    tokenGenerator tg = new tokenGenerator();
                    session.setAttribute("token", tg.generateHash());
                    ...
                    +"&redirect_uri="
                    +redirect
                    +"&response_type="+RESPONSE_TYPE+"&scope="+SCOPE+"&state="
                    +token);
