...
<!--                    <xsl:if test="dri:list[@n=(concat($handle, ':dc.date.issued'))]">
                    ...
                    <xsl:if test="dri:list[@n=(concat($handle, ':dc.coverage'))]">
            
                
                <strong>
                    <i18n:text>País: </i18n:text>
                </strong>
                <xsl:for-each select="dri:list[@n=(concat($handle, ':dc.coverage'))]/dri:item">
                    <a>
                        <xsl:attribute name="href">
                                    <xsl:value-of
        ...
        <xsl:if test="dri:list[@n=(concat($handle, ':dc.contributor.entity'))]">
            
                
                <strong>
                    <i18n:text>Entidad que elaboró el plan: </i18n:text>
                </strong>
        ...
        <xsl:if test="dri:list[@n=(concat($handle, ':dc.doctype.specific'))]">
            
                
                <strong>
                    <i18n:text>Tipo de documento: </i18n:text>
                </strong>
                <xsl:for-each select="dri:list[@n=(concat($handle, ':dc.doctype.specific'))]/dri:item">
                    <a>
                        <xsl:attribute name="href">
                                    <xsl:value-of
        ...
        <xsl:if test="dri:list[@n=(concat($handle, ':dc.subject.spanish'))]">
            
                
                <strong>
                    <i18n:text>Palabras Claves: </i18n:text>
                </strong>
                <xsl:for-each select="dri:list[@n=(concat($handle, ':dc.subject.spanish'))]/dri:item">
                    <a>
                        <xsl:attribute name="href">
                                    <xsl:value-of
    ...
    <xsl:template match="dri:list[@id='aspect.discovery.SimpleSearch.list.primary-search']//dri:item[dri:field[@id='aspect.discovery.SimpleSearch.field.query']]" priority="3">
        ...
        <div class="row">
            <div class="col-sm-offset-0 col-sm-12" id="filters-overview-wrapper-squared"/>
        </div>
    </xsl:template>
